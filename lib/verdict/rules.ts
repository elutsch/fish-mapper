...
const craftThresholds: Record<Craft, { goWind: number; marginalWind: number; goGust: number; marginalGust: number }> = {
  powerboat: { goWind: 30, marginalWind: 44, goGust: 48, marginalGust: 62 },
...
export function fallbackVerdict(
  ...
  validFor: string,
  caveat = "Generated by deterministic fallback because no valid agent verdict was available."
): Verdict {
  ...
  return {
    ...
    summaryMd: `${spot.name} fishes best as a ${lee}-shore plan today: wind over about ${spot.maxFetchKm ?? "unknown"} km of fetch should push the cleanest, most controllable water into that edge. Peak sustained wind near ${Math.round(peakWind)} km/h with gusts near ${Math.round(peakGust)} km/h makes the bite window craft-specific: powerboat anglers can cover the most water, kayak anglers need a short protected drift, and canoe anglers have the narrowest margin. ${pressureText}`,
    caveats: [
      caveat,
      ...(spot.reviewNotes ?? [])
    ]
...
function craftVerdictFor(
  ...
  return {
    rating,
    bestWindow: window,
    note: `${craftText} Prioritize the ${lee} shore for cleaner casts and better boat control.`
  };
}
